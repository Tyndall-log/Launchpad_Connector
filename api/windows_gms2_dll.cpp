...
API void test()
{
	...
	else
	{
		...
		auto message_manager = MessageManager::getInstanceWithoutCreating();
		if (!message_manager)
		{
			log::println("MessageManager::getInstanceWithoutCreating() failed.");
		}
		else
		{
			log::println("MessageManager::getInstanceWithoutCreating() success.");
		}
		auto env = juce::getEnv();
		if (!env)
		{
			log::println("juce::getEnv() failed.");
		}
		else
		{
			log::println("juce::getEnv() success.");
			auto app_context = juce::getAppContext();
			if (!app_context)
			{
				log::println("juce::getAppContext() failed.");
			}
			else
			{
				log::println("juce::getAppContext() success.");
				Thread::initialiseJUCE(env, app_context);
				//launchpad_connector_init();
				//if (!MMT) MMT = make_shared<MainMessageThread>(); //메시지 관리 스레드 시작
				message_manager = MessageManager::getInstanceWithoutCreating();
				if (!message_manager)
				{
					log::println("MessageManager::getInstanceWithoutCreating() failed.");
				}
